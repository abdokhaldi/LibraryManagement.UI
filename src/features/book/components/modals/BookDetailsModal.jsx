...
import { returnBook } from '../../../../services/loanService.js'; 
import ScanModal from '../../../../features/loans/components/modals/ScanModal';
...
function BookDetailsModal({ onClose, onAddCopy: onAddCopyProp }) {
  ...
  const primaryActions = {
    loan: (copy) => {
       // Open ScanModal with the copy's barcode pre-filled
       setSelectedCopyForLoan(copy);
                      ...
                      <td className="p-3 relative">
                        <div className="flex items-center justify-center gap-2">
                          <button  
                            ...
                            onClick={() =>  primaryActions[statusConfig[copy.status.toLowerCase()]?.label.toLowerCase()] (copy.bookCopyID)}
                         >
      ...
      {showScanModal && (
        <ScanModal 
          ...
          onClose={(result) => {
            ...
            setSelectedCopyForLoan(null);
            if (result) {
              // Loan was successful, refresh the data
              loadBookData();
            }
          }}
          initialBarcode={selectedCopyForLoan?.barcode || ""}
        />
      )}
    </>
  );
